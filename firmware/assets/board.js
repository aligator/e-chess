  ...
  fetch("/game-data")
    ...
    .then((data) => {
      ...
      document.getElementById("active-player").textContent =
        `Active player: ${data.activePlayer || "None"}`;
      ...
      if (
        window.requestedGameKey &&
        isLoaded &&
        data.gameKey === window.requestedGameKey
      ) {
...
function disableLoadButton() {
  ...
  if (loadGameButton) {
    ...
    loadGameButton.innerHTML =
      '<span class="button-loading-indicator"></span> Loading...';
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Fill avaliable games
  fetch("/api/game").then(async (res) => {
    const data = await res.json();
    const gameList = document.getElementById("gameKeyList");
    // Add options to the datalist
    gameList.innerHTML = data.open_games
      .map(
        (game) =>
          `<option class="combo-option" value="${game.game_id}">${game.game_id} - ${game.opponent.username}</option>`,
      )
      .join("");
  });

  // setup combo box
  const root = document.getElementById("gameKeyComboDropdown");
  const input = document.getElementById("gameKey");
  const list = document.getElementById("gameKeyList");
  ...
  list.addEventListener("mousedown", (e) => {
    const opt = e.target;
    if (
      !opt.classList.contains("combo-option") ||
