...
use crate::{
    api,
    event::EventManager,
    game::{GameCommandEvent, GameStateEvent, Settings},
    wifi::page,
    Event,
};
...
unsafe fn handle_game(
    server: &mut EspHttpServer,
    current_game_key: Arc<Mutex<String>>,
) -> Result<()> {
        ...
        let html = page(
            html!(
                ...
                div class="game-selector" {
                    ...
                    div class="combo-dropdown" id="gameKeyComboDropdown" {
                        input class="combo-input" id="gameKey" list="availableGames" value=(current_game_key) {}
                        div class="combo-list" id="gameKeyList" {}
                    }

                    datalist id="availableGames" {}

                    button id="loadGame" { "Load Game" }
                    button id="newGame" { "New Local Game" }
                }
            ...
            if let Some(id_param) = query.split('&').find(|p| p.starts_with("key=")) {
                if let Some(id) = id_param.split('=').nth(1) {
                    ...
                    *game_id.lock().unwrap() = "".to_string();
                    ...
                    sender
                        .send(Event::GameCommand(GameCommandEvent::LoadNewGame(
                            id.to_string(),
                        )))
                        .unwrap();
                }
...
impl Web {
    ...
    pub fn register(
        ...
        let tx: mpsc::Sender<Event> = event_manager.create_sender();
